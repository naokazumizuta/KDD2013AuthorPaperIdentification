...
train_deleted <- Train[, c("AuthorId", "DeletedPaperIds")]
names(train_deleted)[2] <- "PaperIds"
train_confirmed <- Train[, c("AuthorId", "ConfirmedPaperIds")]
names(train_confirmed)[2] <- "PaperIds"

train_deleted_long <- toLong(train_deleted)
train_confirmed_long <- toLong(train_confirmed)

train <- rbind(train_deleted_long, train_confirmed_long)
train$target <- c(
    rep(0, nrow(train_deleted_long)),
    rep(1, nrow(train_confirmed_long)))

train$uid <- 1:nrow(train)
...
train_index <- temp_data[, c("uid", "AuthorId", "PaperId")]

setwd(Dir$rdata)
save(train_data, file = "train_data.RData")
save(train_index, file = "train_index.RData")
