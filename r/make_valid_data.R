...
valid_deleted <- Valid[, c("AuthorId", "DeletedPaperIds")]
names(valid_deleted)[2] <- "PaperIds"
valid_confirmed <- Valid[, c("AuthorId", "ConfirmedPaperIds")]
names(valid_confirmed)[2] <- "PaperIds"

valid_deleted_long <- toLong(valid_deleted)
valid_confirmed_long <- toLong(valid_confirmed)

valid <- rbind(valid_deleted_long, valid_confirmed_long)
valid$target <- c(
    rep(0, nrow(valid_deleted_long)),
    rep(1, nrow(valid_confirmed_long)))

valid$uid <- 1:nrow(valid)
...
valid_index <- temp_data[, c("uid", "AuthorId", "PaperId")]

setwd(Dir$rdata)
save(valid_data, file = "valid_data.RData")
save(valid_index, file = "valid_index.RData")
